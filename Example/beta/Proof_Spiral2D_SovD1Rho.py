...
s2 = s2.subs({rho.diff(t,2): 0})
s2 = s2.expand().collect(rho.diff(t,1))
...
save_ccode(sp.Eq(sp.Symbol("double dA"), s2.coeff(rho.diff(t,1),4).subs(dictRep)).simplify())
save_ccode(sp.Eq(sp.Symbol("double dB"), s2.coeff(rho.diff(t,1),2).subs(dictRep)).simplify())
save_ccode(sp.Eq(sp.Symbol("double dC"), s2.coeff(rho.diff(t,1),0).subs(dictRep)-sp.Symbol("dS")**2).simplify())
